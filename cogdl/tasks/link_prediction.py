    ...
    def __init__(self, num_nodes, num_sampled, degree=None):
        ...
        if degree is not None:
            ...
            self.weights = torch.ones(
                (num_nodes,), dtype=torch.float) / num_nodes
    ...
    for walk in walks:
        for i in range(len(walk)):
            for j in range(1, skip_window + 1):
                if i - j >= 0:
                    pairs.append(
                        (vocab[walk[i]].index, vocab[walk[i - j]].index))
                if i + j < len(walk):
                    pairs.append(
                        (vocab[walk[i]].index, vocab[walk[i + j]].index))
    return pairs
...
=======
>>>>>>> a69a969020b8aa41cfcd8ac54511984bc5b32d62
    ...
    for _ in range(num):
        ...
        while True:
            ...
            if trial >= 1000:
                ...
                break
            if x != y and (
                    x, y) not in true_edges_set and (
                    y, x) not in true_edges_set:
                tmp_list.append((x, y))
                break
        if all_flag:
            break
...
def evaluate(embs, true_edges, false_edges):
    ...
    sorted_pred = sorted(prediction_list[:])
...
<<<<<<< HEAD
        self.train_data, self.valid_data, self.test_data = divide_data(edge_list, [
                                                                       0.85, 0.05, 0.10])
...
<<<<<<< HEAD
        ...
        roc_auc, f1_score, pr_auc = evaluate(
            embs, self.test_data[0], self.test_data[1])
=======
